...
class Collection(models.Model):
    ...
    account = models.ForeignKey("Account",verbose_name='用户名')
    date = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return "用户ID：%s --- 文章ID：%s"%(self.account.id,self.content_object.id)
...
class Comment(models.Model):
    ...
    def __str__(self):
        return "%s：---回复内容：%s"%(self.account.username,self.content_object)
...
class UserAuthToken(models.Model):
    ...
    def __str__(self):
        return "%s的token值%s"%(self.user.username,self.token)
